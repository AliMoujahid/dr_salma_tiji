...
class LicenseService {
    ...
    licenseFilePath = path_1.default.join(process.cwd(), 'license.key');
    /**
     * Set custom license file path if needed
     */
    setLicensePath(customPath) {
    ...
    getMachineId() {
        ...
        try {
            if (process.platform === 'win32') {
                ...
                try {
                    const regOutput = (0, child_process_1.execSync)('reg query "HKLM\\SOFTWARE\\Microsoft\\Cryptography" /v MachineGuid', {
                        encoding: 'utf8',
                ...
                try {
                    const wmicOutput = (0, child_process_1.execSync)('powershell -Command "(Get-CimInstance -Class Win32_ComputerSystemProduct).UUID"', {
                        encoding: 'utf8',
                        stdio: ['pipe', 'pipe', 'ignore'],
                    });
        ...
        const hash = crypto_1.default.createHash('sha256').update(rawIdentifier).digest('hex').toUpperCase();
    ...
    verifyLicense(licenseString) {
        ...
        if (!rawKey) {
            const searchPaths = [
                ...
                path_1.default.join(__dirname, '..', '..', 'license.key'),
                path_1.default.join(__dirname, '..', 'license.key'),
                path_1.default.join(process.cwd(), '..', 'license.key'),
            ];
            for (const p of searchPaths) {
                if (fs_1.default.existsSync(p)) {
                    try {
                        rawKey = fs_1.default.readFileSync(p, 'utf8').trim();
                        break;
                    }
                    catch {
    ...
    activate(licenseKey) {
        ...
        try {
            fs_1.default.writeFileSync(this.licenseFilePath, licenseKey.trim(), 'utf8');
