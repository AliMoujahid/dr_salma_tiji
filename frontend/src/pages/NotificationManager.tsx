...
export const NotificationManager: React.FC = () => {
  ...
  const handleSaveSettings = (e: React.FormEvent) => {
    ...
    fetch(`${API_URL}/notifications/settings`, {
      ...
      .then((data) => {
        setSettings(data);
        toast.success('Paramètres enregistrés', 'Les réglages des notifications ont été mis à jour avec succès.');
      })
      .catch((err) => {
    ...
    <div className="p-6 md:p-8 space-y-6 text-slate-900 dark:text-white font-sans">
      ...
      {activeTab === 'Settings' && (
        <form onSubmit={handleSaveSettings} className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="lg:col-span-2 p-6 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200/90 dark:border-white/10 shadow-sm flex flex-col gap-4">
            <div className="flex justify-between items-center border-b border-slate-100 dark:border-white/5 pb-3">
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  <span className={`w-2 h-2 rounded-full ${waStatus?.connected ? 'bg-emerald-500 animate-ping' : 'bg-amber-500'}`}></span>
                  {waStatus?.connected ? `Connecté (${waStatus?.phoneNumber || 'WhatsApp'})` : waStatus?.status || 'Non connecté'}
                </span>
                <button
                  type="button"
                  onClick={handleInitWa}
                  disabled={loadingWaStatus}
                  className="px-3 py-1 rounded-xl bg-blue-600 hover:bg-blue-500 text-white text-xs font-semibold cursor-pointer transition-all disabled:opacity-50"
                >
                  {loadingWaStatus ? 'Connexion...' : 'Relancer WhatsApp'}
                </button>
                {waStatus?.connected && (
                  <button
                    type="button"
                    ...
                    disabled={loadingWaStatus}
                    className="px-3 py-1 rounded-xl bg-rose-600 hover:bg-rose-500 text-white text-xs font-semibold cursor-pointer transition-all disabled:opacity-50"
                  >
            ...
            {waStatus?.qrCodeUrl && !waStatus?.connected && (
              <div className="flex flex-col md:flex-row items-center gap-6 p-4 rounded-xl bg-slate-50 dark:bg-slate-950/40 border border-emerald-500/30">
                <div className="p-3 bg-white rounded-2xl shadow-lg border border-slate-200 shrink-0">
                  <img src={waStatus.qrCodeUrl} alt="WhatsApp Web QR Code" className="w-52 h-52 object-contain" />
                </div>
                <div className="flex flex-col gap-2">
                  <h4 className="text-sm font-bold text-slate-900 dark:text-white flex items-center gap-2">
                    📱 Scanner le QR Code avec WhatsApp ({getWhatsAppTargetNumber()})
                  </h4>
                  <ol className="text-xs text-slate-600 dark:text-slate-300 flex flex-col gap-1.5 list-decimal pl-4">
                    <li>Ouvrez l'application <strong>WhatsApp</strong> sur votre téléphone ({getWhatsAppTargetNumber()}).</li>
                    <li>Allez dans <strong>Réglages / Menu &gt; Appareils connectés (Linked Devices)</strong>.</li>
                    <li>Cliquez sur <strong>Lier un appareil</strong> et scannez le QR Code ci-contre.</li>
                    <li>Une fois scanné, toutes vos notifications de la clinique seront envoyées <strong>GRATUITEMENT</strong> depuis votre propre numéro!</li>
                  </ol>
                </div>
              </div>
            )}
            ...
            {!waStatus?.connected && !waStatus?.qrCodeUrl && (
              <div className="p-4 rounded-xl bg-slate-50 dark:bg-slate-950/40 border border-slate-200 dark:border-white/5 flex flex-col md:flex-row items-center justify-between gap-4">
                <div className="flex flex-col gap-1">
                  <h4 className="text-xs font-bold text-slate-900 dark:text-white">Générer le QR Code d'association WhatsApp Web</h4>
                  <p className="text-xxs text-slate-500 dark:text-slate-400">Cliquez sur le bouton ci-contre pour générer le QR Code à scanner avec votre téléphone ({getWhatsAppTargetNumber()}).</p>
                </div>
                <button
                  type="button"
                  onClick={handleInitWa}
                  disabled={loadingWaStatus}
                  className="px-4 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold transition-all shadow-md shadow-emerald-600/20 cursor-pointer disabled:opacity-50 shrink-0"
                >
                  {loadingWaStatus ? 'Génération en cours...' : '📱 Générer le QR Code'}
                </button>
              </div>
            )}

            {waStatus?.connected && (
              <div className="p-4 rounded-xl bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/20 text-xs font-semibold text-emerald-800 dark:text-emerald-300 flex items-center gap-3">
                <CheckCircle2 className="w-5 h-5 text-emerald-500 shrink-0" />
                <span>
                  WhatsApp Web est connecté et prêt ! Tous les rappels de rendez-vous et messages de diffusion sont envoyés depuis le numéro du cabinet.
                </span>
              </div>
            )}
          </div>
          ...
          <div className="p-6 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200/90 dark:border-white/10 shadow-sm flex flex-col gap-4">
            <h3 className="text-sm font-bold text-slate-900 dark:text-white flex items-center gap-2 border-b border-slate-100 dark:border-white/5 pb-3">
              <MessageSquare className="w-4 h-4 text-emerald-600 dark:text-emerald-400" /> Paramètres d'Envoi WhatsApp
            </h3>

            <div className="flex justify-between items-center">
              <span className="text-xs text-slate-700 dark:text-slate-300 font-semibold">Activer les rappels WhatsApp</span>
              <input
                type="checkbox"
            ...
            <div className="p-3 bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/20 rounded-xl text-xxs text-emerald-800 dark:text-emerald-300 leading-relaxed">
              ✅ <strong>Mode WhatsApp Web Gratuit Actif :</strong> Aucune clé Meta ou Twilio n'est nécessaire. Scannez simplement le QR Code ci-dessus avec votre téléphone pour envoyer vos messages gratuitement !
            </div>
          </div>
          ...
          {isAdmin && (
            <div className="p-6 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200/90 dark:border-white/10 shadow-sm flex flex-col gap-4">
              <h3 className="text-sm font-bold text-slate-900 dark:text-white flex items-center gap-2 border-b border-slate-100 dark:border-white/5 pb-3">
                <ShieldCheck className="w-4 h-4 text-rose-500" /> Mode de Test & Simulation (Admin)
              </h3>
              
              <div className="flex justify-between items-center">
                <div className="flex flex-col gap-0.5">
                  <span className="text-xs text-slate-700 dark:text-slate-300 font-semibold">Mode Test Actif</span>
                  <span className="text-[10px] text-slate-500 dark:text-slate-400">Si activé, tous les messages seront redirigés vers le numéro de test ci-dessous.</span>
                </div>
                <input
                  type="checkbox"
              ...
              <div>
                <label className="block text-xxs font-semibold text-slate-600 dark:text-slate-400 mb-1">Numéro de Téléphone de Test (ex: +2126XXXXXXXX)</label>
                <input
                  type="text"
                  placeholder="+212613117131"
                  value={settings.testPhoneNumber || ''}
                  onChange={(e) => setSettings({ ...settings, testPhoneNumber: e.target.value })}
                  className="w-full h-10 px-3 rounded-xl text-xs border border-slate-200 dark:border-white/10 bg-white dark:bg-slate-950 text-slate-900 dark:text-white focus:outline-none focus:border-blue-500 shadow-xs"
                />
              </div>
            </div>
          )}
